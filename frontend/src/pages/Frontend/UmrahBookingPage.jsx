...
import { submitZipUmrahBooking } from "../../api/umrahBookingApi";
...
export default function UmrahBookingPage({ user }) {
  ...
  const addAdult = () => {
    if (isSharing) {
      if (formData.adults.length + formData.children.length >= maxAdults) {
        toast.warning(
          `Maximum ${maxAdults} passengers (adults + children) allowed based on available rooms`,
        );
        return;
      }
    } else if (formData.adults.length >= maxAdults) {
      toast.warning(
  ...
  const addChild = () => {
    if (isSharing && formData.adults.length + formData.children.length >= maxAdults) {
      toast.warning(
        `Maximum ${maxAdults} passengers (adults + children) allowed based on available rooms`,
      );
      return;
  ...
  const confirmAndSubmit = async () => {
    ...
    try {
      ...
      // Step 1: Upload each passport file and collect URLs
      const passengersWithUrls = await Promise.all(
        allPassengers.map(async (p, i) => {
          let documentUrl = "";
          if (p.passportFile) {
            const uploadFd = new FormData();
            uploadFd.append("file", p.passportFile);
      ...
      // Step 2: Build JSON payload for zip-accounts/submit-booking
      const payload = {
        type: "UmrahPackage",
        packageId:
          packageData._id ||
        ...
        passengers: passengersWithUrls,
        originalPkg: packageData,
      };

      const res = await submitZipUmrahBooking(payload);
      if (res.success) {
        toast.success("Booking submitted! Booking#: " + res.bookingNumber);
            ...
            >
              ...
              {isSharing && (
                <button
                  type="button"
                  onClick={addAdult}
                  style={s.btn("#48bb78", isSharing ? formData.adults.length + formData.children.length >= maxAdults : formData.adults.length >= maxAdults)}
                  disabled={isSharing ? formData.adults.length + formData.children.length >= maxAdults : formData.adults.length >= maxAdults}
                >

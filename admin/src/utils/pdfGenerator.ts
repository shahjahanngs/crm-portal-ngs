...
/**
 * Interface representing the booking structure for type safety
 */
interface Booking {
  pnr?: string;
...
export const generateBookingPDF = async (booking: Booking): Promise<void> => {
  ...
  // Fully dynamic mapping (Ditto Logic from JS)
  ...
  element.innerHTML = `
    <div>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <div><img src="/src/assets/images/logo.webp" alt="Agency Logo" style="width: 140px; object-fit: contain;" /></div>
            <div><img src="${airlineLogo}" alt="Airline Logo" style="height: 70px; object-fit: contain;" /></div>
        </div>
        ...
        <div style="margin-bottom: 30px; padding: 20px 0; border-bottom: 1px solid #ddd;">
            ...
            <div style="display: flex; align-items: flex-start; position: relative; color: #000;">
                <div style="width: 18%; font-size: 16px;"><div>${airlineName}</div></div>
                <div style="width: 20%;">
  ...
  try {
    ...
    const finalFileName = `${sanitizeFilename(reference)}_${sanitizeFilename(`${oCode}-${dCode}`)}.pdf`;
...
// --- Helper Functions (Identical to JS version) ---
